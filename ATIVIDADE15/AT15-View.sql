...
select nome_cliente, year(data_nascimento) as
ano_nascimento, month(data_nascimento) as
mes_nascimento
from tb_cliente;
...
from vw_produtos_preco
where lucro > 5.00;

select categoria, quantidade_produtos
from vw_categorias
...
create view vw_produtos_lucros as 
select nome_produto, categoria, (preco_venda - preco_compra) as lucro
from tb_produto
where estoque > 50;
 ...
 SELECT c.nome_cliente, c.telefone_cliente, c.cidade, c.estado,
 SUM((p.preco_venda - p.preco_compra) * vp.quantidade) AS
 lucro_total 
 FROM tb_cliente c
 JOIN tb_venda v 
 ON c.id_cliente = v.CLIENTE_id_cliente 
 JOIN tb_venda_produto vp
 ON v.id_venda = vp.VENDA_id_venda 
 JOIN tb_produto p ON vp.PRODUTO_produto_id =
 p.id_produto 
 ...
 SELECT cl.nome_cliente, cl.lucro_total 
 FROM vw_clientes_mais_lucrativos cl 
 JOIN vw_produtos_lucros pl ON pl.categoria = 
 'Alimentos' 
 WHERE cl.lucro_total > 100 
 ORDER BY cl.lucro_total DESC;
